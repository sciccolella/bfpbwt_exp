...
# --- Configuration ---
DATA_DIR = "/data/proj/2bfpbwt/one_thousands/phase3"

# Executables
BCF_EXE = "/data/proj/2bfpbwt/bf-pbwt-clean/2bfpbwt-bcf"
BM_EXE = "/data/proj/2bfpbwt/bf-pbwt-clean/2bfpbwt-bm"
V2B_EXE = "/data/proj/2bfpbwt/haploblocks/vcf2bm/vcf2bm"
PBWT_EXE = "/data/proj/2bfpbwt/pbwt/pbwt"
PAR_PBWT_EXE = "/data/proj/2bfpbwt/parallel_pbwt/app"
SYLLABLE_EXE = "/data/proj/2bfpbwt/exp/Syllable-PBWT/server"
...
rule all:
    input:
        ...
        expand(os.path.join(DATA_DIR, "{sample}.bm"), sample=SAMPLES),
...
rule stack_times:
    input:
        expand("results/1kp3.{sample}.{tool}.time.csv", sample=SAMPLES,
               tool=["bcf.ars", "bcf.lin", "bcf.prs", "bcf.spr", 
                     "bm.barm", "bm.bars", "bm.blim", "bm.blis", 
                     "bm.bpr", "bm.bprs", "bm.bprm", 
                     "bm.prs", "bm.spr", "bm.sprs", "bm.sprm", 
                     "parpbwt", "pbwt", "syl"]),
    output:
        "results/all.csv"
    shell:
...
rule run_bf_pbwt_bm:
    ...
    wildcard_constraints:
        arg = "[a-zA-Z0-9]+"
    threads: lambda wildcards: 8 if wildcards.arg in ["bpr","bprs","bprm", "spr", "prs", "sprs", "sprm"] else 1
    shell:
...
rule run_parallel_pbwt:
    ...
    shell:
        """
        LD_LIBRARY_PATH=/home/ciccolella/libs/htslib/lib \\time -vo {output.time} {PAR_PBWT_EXE} -f {input.bcf} -o {params.tmp_txt} -t {threads} > {output.out}
